...
SkipList<KeyType>::SkipList(int _size, std::vector<KeyType> keys)
{
    ...
    {
        level = 0;
        while (level < _height - 1 and pow(2, level) % keyIndex != 0 and keyIndex % 2 == 0)
            level++;
        int i = 0;
        while (i <= level)
        {
            newNode = new Node(keys[keyIndex], ValType::VAL);
